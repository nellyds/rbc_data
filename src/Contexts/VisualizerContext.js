...
export default class VisualizerContextProvider extends React.Component {
    ...
    getSorts = (index) => {
        ...
        this.setState({parseErrors: [...this.state.errors, 'Data could not be sorted']})
    }
    }
    getColumnSet = (index) => {
    ...
    compileForChart = async (dataKey, color) => {
        try{
            ...
            this.setState({parseErrors: [...this.state.errors, 'could not compile chart']})
        }
    ...
    render() {
        return (
            <VisualizerContext.Provider value={{
                ...this.state, getDataKeys: this.getDataKeys, compileForChart: this.compileForChart,
